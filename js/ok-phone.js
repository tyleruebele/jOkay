 ...
 */
function okPhone(phone, beStrict, beWordy) {
...
okPhone.input = function(Input, beStrict, beWordy) {
    ...
    if ('undefined' === typeof beWordy) {
        beWordy = !!Input.className.match(/js-ok-phone-wordy/);
    }
    ...
    if ('' != Input.value && false === phone) {
        ...
        Input.title = okPhone.areaCodes[phone.substr(0,3)] || '';
        Input.value = phone;
    }
};
...
/**
 * Hold the error number produced by okPhone, or 0 for no error
 * @type {number}
 */
okPhone.errno = 0;

/**
 * okPhone response code messages
 * Customized script by altering this before okPhone is called
 * @type {Array}
 */
okPhone.errors = [
 ...
 * Customized script by altering this before okPhone.init is called
 * @type {string}
 */
okPhone.query = 'input[type=tel],'
    + 'input.js-ok-phone,'
    + 'input.js-ok-phone-loose,'
    + 'input.js-ok-phone-strict,'
    + 'input.js-ok-phone-wordy'
;
 ...
 * @type {string}
 */
okPhone.errorClass = 'js-ok-phone-error';
...
// Run okPhone.init() when the page loads
window.addEventListener
    ? window.addEventListener('load', okPhone.init, false)
    : window.attachEvent && window.attachEvent('onload', okPhone.init)
;
