...
function validateEmail(email, beStrict, beStricter) {
    ...
    if (Parts = domain.match(DomainIPRE)) {
        if (Parts[1] > 255 || Parts[2] > 255 || Parts[3] > 255 || Parts[4] > 255) {
            validateEmail.errno = 4;
            return false;
        } else {
            validateEmail.errno = 0;
            return email;
        }
    }
...
validateEmail.input = function(Input, beStrict, beStricter) {
    ...
    var email = validateEmail(Input.value, beStrict, beStricter);
    if ('' != Input.value && false === email) {
        Input.title = validateEmail.errors[validateEmail.errno];
...
/**
 * Hold the error number produced by validateEmail, or 0 for no error
 * @type {number}
 */
validateEmail.errno = 0;
